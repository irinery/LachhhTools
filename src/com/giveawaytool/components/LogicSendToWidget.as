package com.giveawaytool.components {
	...
	public class LogicSendToWidget extends ActorComponent {
		...
		public var MODEL_YOUTUBE : String = "youtube";
		...
		public function setModelForYoutube() : void {
			modelAPIToGetCodeFrom = MODEL_YOUTUBE;
		}
		...
		private function widgetSocket_dataHandler( event:ProgressEvent ):void {
			...
			if(msg.toString().indexOf("GET /?") == 0) {
				...
				} else if(modelAPIToGetCodeFrom == MODEL_YOUTUBE) {
					if(YoutubeConnection.instance != null) {
						YoutubeConnection.instance.setCodeFromWebSocket(code, state);
					}
				}
				///clearShitOnURL();
